  ...
  if (req.session.user) {
    try {
      const user = await User.query()
        .select("id", "username", "email")//include this fields
        .findById(req.session.user.id);// find it by id
...
router.post("/register", async (req, res, next) => {
  ...
  if (username && email) {
    ...
    if (userExists.length) { // checking for username and email exist or not
      return res
        .status(400)
